                      ...
                      {itensBusca.map(item => (
                        <div key={item.id + item.catKey} style={{ background: '#fafafa', borderRadius: 8, padding: 12, borderLeft: `4px solid ${item.catCor}`, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                          ...
                          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                            ...
                            <input type="text" inputMode="numeric" pattern="[0-9]*" value={quantidades[item.id] || 1} onChange={e => { const v = e.target.value; setQuantidades({...quantidades, [item.id]: v === '' ? '' : Math.max(1, parseInt(v)||1)}) }} style={{ width: 44, textAlign: 'center', padding: '3px', border: '1px solid #ddd', borderRadius: 4, fontSize: 13 }}/>
              ...
              return (
                <div key={item.id} style={{ background: 'white', borderRadius: 10, padding: 14, boxShadow: '0 2px 8px rgba(0,0,0,0.08)', borderLeft: `4px solid ${cat.cor}` }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    {item.pesoCustom ? (
                      <><input type="text" inputMode="numeric" pattern="[0-9]*" value={quantidades[item.id] || 100} onChange={e => { const v = e.target.value; setQuantidades({...quantidades, [item.id]: v === '' ? '' : Math.max(1, parseInt(v)||1)}) }} style={{ width: 70, textAlign: 'center', padding: '4px', border: `1px solid ${cat.cor}`, borderRadius: 4 }}/><span style={{fontSize:12,color:'#888'}}>g</span></>
                    ) : (
                    ...
                    <input type="text" inputMode="numeric" pattern="[0-9]*" value={quantidades[item.id] || 1} onChange={e => { const v = e.target.value; setQuantidades({...quantidades, [item.id]: v === '' ? '' : Math.max(1, parseInt(v)||1)}) }} style={{ width: 50, textAlign: 'center', padding: '4px', border: '1px solid #ddd', borderRadius: 4 }}/>
